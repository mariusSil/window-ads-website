...
interface TechnicianRequestFormProps {
  locale: Locale;
  translations?: FormTranslations;
  prefillMessage?: string;
  triggerType?: 'technician' | 'consultation' | 'learnMore' | 'chat';
...
export function TechnicianRequestForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        onSubmitError?.(result.error || 'UNKNOWN_ERROR')
      }
    } catch (error) {
      console.error('Network error:', error)
      setSubmitStatus('error')
      onSubmitError?.('NETWORK_ERROR')
    } finally {
  ...
  const statusMargin = compact ? 'mb-3' : 'mb-4'

  return (
    <div className={className}>
      {showTitle && (
        <h3 className={`font-semibold text-secondary ${compact ? 'text-lg mb-3' : 'text-xl mb-4'}`}>
          {finalTranslations.title}
        </h3>
      )}
      
      {/* Status Messages */}
      {submitStatus === 'success' && (
        <div className={`${statusMargin} p-3 bg-green-50 border border-green-200 rounded-lg`}>
      ...
      {submitStatus === 'error' && (
        <div className={`${statusMargin} p-3 bg-red-50 border border-red-200 rounded-lg`}>
          <div className="flex items-center">
            <Icon name="Info" className="w-4 h-4 text-red-600 mr-2 flex-shrink-0" />
      ...
      {submitStatus === 'rate-limited' && (
        <div className={`${statusMargin} p-3 bg-yellow-50 border border-yellow-200 rounded-lg`}>
          <div className="flex items-center">
            <Icon name="Clock" className="w-4 h-4 text-yellow-600 mr-2 flex-shrink-0" />
      ...
      <form onSubmit={handleSubmit} className={formSpacing}>
        ...
        <div>
          <Label htmlFor="name" className={compact ? 'text-sm' : ''}>{finalTranslations.nameLabel}</Label>
          <Input 
            id="name" 
            name="name" 
            placeholder={finalTranslations.namePlaceholder} 
            required 
        ...
        <div>
          <Label htmlFor="phone" className={compact ? 'text-sm' : ''}>{finalTranslations.phoneLabel}</Label>
          <Input 
            id="phone" 
            name="phone" 
            type="tel" 
            placeholder={finalTranslations.phonePlaceholder} 
            required 
            className={compact ? 'h-9' : ''}
          />
        </div>
        ...
        <div>
          <Label htmlFor="city" className={compact ? 'text-sm' : ''}>{finalTranslations.cityLabel}</Label>
          <Input 
            id="city" 
            name="city" 
            placeholder={finalTranslations.cityPlaceholder} 
            className={compact ? 'h-9' : ''}
          />
        </div>
        
        <div>
          <Label htmlFor="email" className={compact ? 'text-sm' : ''}>{finalTranslations.emailLabel}</Label>
          <Input 
            id="email" 
            name="email" 
            type="email" 
            placeholder={finalTranslations.emailPlaceholder} 
            className={compact ? 'h-9' : ''}
          />
        </div>
        
        <div>
          <Label htmlFor="message" className={compact ? 'text-sm' : ''}>{finalTranslations.messageLabel}</Label>
          <Textarea 
            id="message" 
            name="message" 
        ...
        <div className="flex items-start space-x-2">
          ...
          <Label htmlFor="privacy" className={`leading-tight ${compact ? 'text-xs' : 'text-sm'}`}>
        ...
        >
          {isSubmitting ? 
            (locale === 'lt' ? 'Siunčiama...' : 
             ...
             'Sending...') : 
            finalTranslations.submitButton
          }
        </Button>
      </form>
    </div>
